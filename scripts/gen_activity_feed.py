"""Writes the last 5 public GitHub events into README.md between the
...
def main():
    events = fetch_events()
    lines = [f"- {fmt(e)}" for e in events[:5]]
    if not lines:
        lines = ["- ⛵ *No public activity yet — the Journal fills in as soon as there's something to log.*"]
